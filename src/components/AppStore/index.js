...
class AppStore extends Component {
  ...
  render() {
    ...
    const filteredTabAppsList = this.getFilteredTabAppsList(activeTab)
    const searchResults = filteredTabAppsList.filter(each =>
      ...
      <div className="bg-container">
        ...
        <ul className="tabs-ul">
          {tabsList.map(each => (
            <TabItem
              tabDetails={each}
              updateAppsOnPage={this.updateAppsOnPage}
              key={each.tabId}
            />
          ))}
        </ul>
